...
export async function POST(req: NextRequest) {
  try {
    ...
    const { trade, region, county, name, phone } = body;

    if (!trade || !region || !county || !name || !phone) {
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  <div style="background: #111; padding: 24px; border-bottom: 3px solid #FF6B00;">
    <h1 style="color: #FF6B00; margin: 0; font-size: 24px;">New Territory Claim</h1>
    <p style="color: #999; margin: 8px 0 0;">${name} &mdash; ${trade} in ${county}</p>
  </div>

  <div style="padding: 24px; background: #1a1a1a; color: #ddd;">
    <h2 style="color: #FF6B00; font-size: 16px; margin: 0 0 12px; text-transform: uppercase;">Partner Details</h2>
    <table style="width: 100%; border-collapse: collapse;">
      ...
      <tr>
        <td style="padding: 8px 0; color: #999;">County (Territory)</td>
        <td style="padding: 8px 0; text-align: right; color: #FF6B00; font-weight: bold;">${county}</td>
      </tr>
    </table>
  </div>

  <div style="padding: 16px 24px; background: #111; text-align: center;">
    <p style="color: #666; margin: 0; font-size: 12px;">4A Trades &mdash; Territory Partnership Application</p>
  </div>
</div>`;

    const resend = new Resend(process.env.RESEND_API_KEY);
    const { error } = await resend.emails.send({
      from: "4A Trades Territories <orders@4atrades.co.uk>",
      to: ["vincent@askqs.co.uk", "chris@4atrades.co.uk"],
      subject: `New Territory Claim: ${name} — ${trade} in ${county}`,
      html,
    });
