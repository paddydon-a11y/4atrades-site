...
const categories = ["General & Civil", "Building & Fit-Out", "Management & Technical"] as const;

export default function TradesIndexContent() {
  return (
    <main>
      {/* Hero */}
      <section className="relative py-32 pt-40 px-6 overflow-hidden bg-darker noise-bg">
        ...
        <div className="relative mx-auto max-w-5xl text-center">
          ...
          >
            {allTrades.length} construction trades across every discipline. From site labourers to
          ...
          <div className="mt-16 grid md:grid-cols-2 gap-8">
            {[
              ...
              <AnimatedSection key={item.title} delay={i * 0.1}>
                <motion.div whileHover={{ x: 4 }} className="border-l-2 border-accent pl-6 py-4">
      ...
      {/* Trades Grid by Category */}
      <section className="bg-dark geo-pattern py-24 px-6">
        <div className="mx-auto max-w-7xl">
          {categories.map((category, ci) => {
            const trades = allTrades.filter((t) => t.category === category);
            return (
              <div key={category} className={ci > 0 ? "mt-20" : ""}>
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {trades.map((trade, ti) => (
                    <AnimatedSection key={trade.slug} delay={ti * 0.03}>
